...
3. **equity 범위 검증**: 개별 equity_share_bps (1-10000) + 콘테스트 합계 ≤ 10000 (EquityOverflow)
4. **트리 바인딩**: merkle_tree == contest_tree.merkle_tree 검증 + bubblegum_tree_config PDA 파생 검증
5. **불변 메타데이터**: cNFT leaf 데이터는 온체인 Merkle Tree에 해시로 저장 → 변조 불가
6. **Phase 2와 독립**: 별도 Program ID, 별도 config PDA → Phase 2에 영향 없음

> **참고:** `equity_share_bps`, `role` 등 off-chain 메타데이터 스키마의 유효성은 온체인에서 검증하지 않습니다.
> 이 검증은 백엔드 mint 파이프라인(서버)에서 수행합니다.
> 온체인은 총 equity 합계 초과만 방지합니다.

## 사전 조건

Phase 3 admin 작업 전에 반드시 `initialize` instruction을 1회 호출해야 합니다.
이후 initialize_contest_tree, mint_participant_nft, finalize_contest 사용 가능.
