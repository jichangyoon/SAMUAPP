import { 
  memes, votes, nfts, nftComments, partnerMemes, partnerVotes, users, contests, archivedMemes,
  type Meme, type InsertMeme, type Vote, type InsertVote, type Nft, type InsertNft, 
  type NftComment, type InsertNftComment, type PartnerMeme, type InsertPartnerMeme, 
  type PartnerVote, type InsertPartnerVote, type User, type InsertUser,
  type Contest, type InsertContest, type ArchivedMeme, type InsertArchivedMeme
} from "@shared/schema";
...
export class DatabaseStorage implements IStorage {
  ...
  async startContest(contestId: number): Promise<Contest> {
    if (!this.db) throw new Error("Database not available");
    
    // End any currently active contest first
    await this.db
      .update(contests)
      .set({ status: 'ended' })
      .where(eq(contests.status, 'active'));
  ...
  async endContestAndArchive(contestId: number): Promise<{ contest: Contest; archivedMemes: ArchivedMeme[] }> {
    if (!this.db) throw new Error("Database not available");
    ...
    // Get all current memes to archive
    const currentMemes = await this.db
      .select()
      .from(memes)
      .orderBy(desc(memes.votes));

    // Archive memes with ranking
    const archivedMemeInserts = currentMemes.map((meme, index) => ({
      contestId,
      ...
      authorWallet: meme.authorWallet,
      authorUsername: meme.authorUsername,
      authorAvatarUrl: meme.authorAvatarUrl,
      votes: Number(meme.votes),
      finalRank: index + 1,
    }));

    // Insert archived memes
    let archivedMemeResults: ArchivedMeme[] = [];
    if (archivedMemeInserts.length > 0) {
      archivedMemeResults = await this.db
        .insert(archivedMemes)
        .values(archivedMemeInserts)
        .returning();
    }

    // Clear current memes and votes for new contest
    await this.db.delete(votes);
    await this.db.delete(memes);

    // Update contest status
    const [updatedContest] = await this.db
      .update(contests)
      .set({ 
        status: 'archived',
        endDate: new Date(),
    ...
    return {
      ...
      archivedMemes: archivedMemeResults
    };
  }

  async isUserAdmin(walletAddress: string): Promise<boolean> {
    if (!this.db) {
      // For development, allow specific wallet as admin
      return walletAddress === 'xfSWSv7y3SqELDe8Xs5neNCmjULpc6hwhvz5ohSrXa8';
    }
  ...
  async getArchivedMemes(contestId?: number): Promise<ArchivedMeme[]> {
    if (!this.db) return [];
    ...
    if (contestId) {
      return await this.db
        .select()
        .from(archivedMemes)
        .where(eq(archivedMemes.contestId, contestId))
        .orderBy(archivedMemes.finalRank);
    }
    
    return await this.db
      .select()
      .from(archivedMemes)
      .orderBy(desc(archivedMemes.archivedAt));
  }
  ...
  async getArchivedContests(): Promise<Contest[]> {
    ...
    return await this.db
      .select()
      .from(contests)
      .where(eq(contests.status, 'archived'))
      .orderBy(desc(contests.archivedAt));
  }
}
