...
## Devnet 테스트 전략 (2026-03-17 확정)

### 배경
Mainnet 배포는 프로그램 바이너리 크기 때문에 **6~7 SOL**이 필요하여 당장 부담스러움. 해커톤 데모 목적으로 **Devnet에서 전체 흐름을 먼저 검증**하고, 제출 직전에 Mainnet 배포 진행.

### Phase 2 Devnet 전략
**목표:** 실제 앱에서 굿즈 구매 → 배송 완료 → 스마트 컨트랙트 claim 전체 흐름 end-to-end 검증

**전체 흐름 (최신):**
1. 유저가 굿즈 결제 → 수익 → escrow_pool PDA로 직접 입금
2. 배송 완료 → 서버가 `initialize_pool` + `deposit_profit` 호출 (서버 가스비 2회)
3. 유저가 Claim 클릭 → 서버가 `record_allocation + claim` 합산 TX 빌드 + admin pre-sign → 유저 지갑 서명 & 제출 (유저 가스비)

**설정:**
- RPC를 devnet으로 전환 (`https://api.devnet.solana.com`)
- `SAMU_REWARDS_PROGRAM_ID` = 재배포 후 새 devnet Program ID로 업데이트 필요
- 에스크로 지갑에 devnet SOL 에어드랍 (initialize_pool + deposit_profit 가스비용)
- 나머지 기능(투표, Printful 등)은 영향 최소화

**주의:** 앱 devnet 전환 시 Privy 지갑 주소는 동일하나 mainnet SAMU/SOL 잔액은 안 보임. 데모용으로만 사용.
...
- 해커톤 데모에서 SAMU 투표 장면은 mainnet 화면 캡처 / Phase 3는 devnet 데모로 분리

### Mainnet 배포 계획
- **시기:** 해커톤 제출 직전 (여유 있을 때)
- **필요 SOL:** Phase 2 컨트랙트 6~7 SOL, Phase 3 컨트랙트 별도 예상 (비슷한 수준)
- **방법:** Solana Playground 대신 **Anchor CLI** 사용 권장 (더 안정적)
- **배포 후:** 에스크로 지갑에 mainnet SOL 있으면 프로그램 계정에 locked된 SOL 일부 회수 가능
